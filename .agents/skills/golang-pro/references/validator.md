...
Use `github.com/global-torque/go-common/validator/v2` for request DTOs and boundary data. It wraps `github.com/go-playground/validator/v10` and returns this repo's `response.Error` shape, so callers get field-keyed JSON errors instead of raw validator messages.
